...
//let configPromise = fetch('./data/config.json')
//	.then((response) => response.json())
	//.then((config) => console.log(data));

//let csvPromise = d3.csv('./data/df_ex.csv')

//const promises = new Map();

//promises.set('config', configPromise);
//promises.set('csv', csvPromise);

//Promise.all(promises).then((dataset) => {
//	plotFigure1(dataset.csv, "", dataset.config);
//});
const config = JSON.parse(document.getElementById('config').textContent);
console.log(document.getElementById('datacsv').textContent)
const dataset = d3.csvParse(document.getElementById('datacsv').textContent);
console.log(dataset)
const age_pickers = [{"id": 0, "value": "0"}, {"id": 1, "value": "60"}];

plotFigure1(dataset, config["routes"], config)
...
function plotFigure1(dataset, routes, config) {
	...
	//let width = $(`#${figureID}-chart-area`).width();
	//let height = $(`#${figureID}-chart-area`).height();
	//let width = $(`#${figureID}`).width();
	let width = document.getElementById(figureID).offsetWidth;
	//let height = $(`#${figureID}`).height() - $(`#${figureID}-chart-caption`).height();
	let height = document.getElementById(figureID).offsetHeight - document.getElementById(`${figureID}-chart-caption`).offsetHeight;
	...
	//.attr('filter', 'drop-shadow(1px 1px 0px black) drop-shadow(-1px 1px 0px black) drop-shadow(1px -1px 0px black) drop-shadow(-1px -1px 0px black)')
	//yaxis.selectAll("g")
	//.append("rect")
	//.attr('width', '25px')
	//.attr('height', y.bandwidth() - 1)
	...
	const rowmouseover = function (event, d) {
		d3.select(this)
			.attr("fill", "black")
			.style("opacity", 0.5)
	};

	const rowmouseleave = function (event, d) {
		d3.select(this)
	...
	selectline.attr("fill", "black")
		...
		.attr('class', 'd-none')
	// create a tooltip
	const Tooltip = d3.select(`#${figureID}-chart-area`)
		.append("div")
		//.style("opacity", 0)
	...
	const mousemove = function (event, d) {
		...
		//selectline.attr('x', bound.left - ((bound.right - bound.left) / 2))
